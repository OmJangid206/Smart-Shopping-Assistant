...
class SupabaseUserBackend:
    ...
    def __init__(self, url: str, key: str, table: str = "users"):
        ...
        self._table = table
        self._client.table(self._table).select("user_id").limit(1).execute()
...
def _make_backend():
    ...
    if use_supabase:
        if not (SUPABASE_URL and SUPABASE_KEY):
            print("[auth] SESSION_BACKEND=supabase but SUPABASE_URL/KEY missing; using memory.")
            return MemoryUserBackend()
        try:
            backend = SupabaseUserBackend(SUPABASE_URL, SUPABASE_KEY)
            print("[auth] using Supabase for user accounts.")
            return backend
        except Exception as e:  # noqa - keep auth from taking the app down
            print(f"[auth] Supabase unavailable ({e}); falling back to in-memory users.")
            return MemoryUserBackend()
    return MemoryUserBackend()
